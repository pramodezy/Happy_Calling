-- ============================================================================
-- MOTOROLA HAPPY CALLING: ADD & POPULATE WARRANTY STATUS & REPAIR TYPE
-- Run this in Supabase Dashboard -> SQL Editor -> New Query -> Run
-- ============================================================================

-- 1. ADD COLUMNS TO closure_master (if not already added)
...
-- 2. INSTANTLY POPULATE COLUMNS FROM EXISTING source_data JSON
-- (Extracts from existing closure uploads so no re-upload is needed!)
...
SET 
    warranty_status = COALESCE(
        ...
        source_data->>'warranty_type',
        source_data->>'IW/OOW',
        source_data->>'IW / OOW',
        source_data->>'IW_OOW',
        source_data->>'In Warranty/Out of Warranty',
        source_data->>'In Warranty',
        source_data->>'Warranty Condition',
        source_data->>'Warranty Category',
        source_data->>'Warranty Description'
    ),
    repair_type = COALESCE(
        ...
        source_data->>'type_of_repair',
        source_data->>'Service Type',
        source_data->>'service_type',
        source_data->>'Repair Nature',
        source_data->>'Action Taken',
        source_data->>'action_taken',
        source_data->>'Repair Description',
        source_data->>'Repair Category',
        source_data->>'Defect Type',
        source_data->>'Fault Type'
    )
WHERE warranty_status IS NULL OR repair_type IS NULL;

-- 3. UPDATE import_closures_batch() RPC FOR FUTURE IMPORTS
...
BEGIN
    ...
    LOOP
        BEGIN
            ...
            -- Extract warranty status & repair type directly or fallback to source_data
            v_warranty_status := COALESCE(
                NULLIF(TRIM(v_item->>'warranty_status'), ''),
                v_source_data->>'Warranty Status',
                v_source_data->>'warranty_status',
                v_source_data->>'Warranty',
                v_source_data->>'IW/OOW',
                v_source_data->>'Warranty Type'
            );

            v_repair_type := COALESCE(
                NULLIF(TRIM(v_item->>'repair_type'), ''),
                v_source_data->>'Repair Type',
                v_source_data->>'repair_type',
                v_source_data->>'Job Type',
                v_source_data->>'Type of Repair',
                v_source_data->>'Service Type',
                v_source_data->>'Action Taken'
            );
...
-- 4. UPDATE get_next_pending_closure() TO RETURN WARRANTY & REPAIR TYPE
...
-- 5. PURGE PREVIOUS DEMO CCIs IF REAL STATIONS EXIST
DO $$
BEGIN
    IF EXISTS (
